...
let rec print_number number =
    if (String.length (string_of_bigint number)) > 69
        ...
        (*    printf "this ran\n"; *)
            ...
            print_number
            (bigint_of_string(String.sub (string_of_bigint number) 69
...
let register_arr = Array.make 256 Bigint.zero

let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' -> push (Array.get register_arr reg) thestack
        | 's' -> Array.set register_arr reg (pop thestack)
        | _   -> printf
        "0%o 0%o is unimplemented in executereg \n%!" (ord oper) reg
...
let execute (thestack: stack_t) (oper: char) =
    try match oper with
        ...
        | ' '  -> ()
        | _    -> printf
                  "0%o is unimplemented in execute\n%!" (ord oper)
    with Stack.Empty -> print_stackempty()
